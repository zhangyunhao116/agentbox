...
- **classifier**: `classifyCtx` precomputation — `classifyOnce` now computes `strings.Fields`, `strings.ToLower`, and `baseCommand` once per call and shares the results across all rules via the new `MatchCtx` rule field. This eliminates 71+ redundant `strings.Fields` calls, 32+ `strings.ToLower` calls, and 117+ `baseCommand` calls per worst-case classification. The `MatchCtx` field is additive and backward-compatible: rules may implement `MatchCtx`, `Match`, or both. Migrated 7 escalated rules to use the precomputed context, with the framework ready for further migration.
